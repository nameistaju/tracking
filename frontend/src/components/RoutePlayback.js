...
function FollowMarker({ position, shouldFollow }) {
  const map = useMap();
  useEffect(() => {
    if (shouldFollow && position) map.panTo(position, { animate: true, duration: 0.3 });
...
export default function RoutePlayback({ logs = [], userName = "" }) {
  ...
  useEffect(() => { injectStyles(); setMounted(true); return () => { if (intervalRef.current) clearInterval(intervalRef.current); }; }, []);

  useEffect(() => {
    if (intervalRef.current) clearInterval(intervalRef.current);
    if (isPlaying && currentIndex < logs.length - 1) {
      intervalRef.current = setInterval(() => {
        setCurrentIndex(prev => { if (prev >= logs.length - 1) { setIsPlaying(false); return prev; } return prev + 1; });
      }, SPEED_OPTIONS[speedIndex].value);
    }
    return () => { if (intervalRef.current) clearInterval(intervalRef.current); };
  }, [isPlaying, speedIndex, logs.length, currentIndex]);

  useEffect(() => { if (currentIndex >= logs.length - 1) setIsPlaying(false); }, [currentIndex, logs.length]);

  const positions = useMemo(() => logs.map(l => [l.location.lat, l.location.lng]), [logs]);
  const trailPositions = useMemo(() => positions.slice(0, currentIndex + 1), [positions, currentIndex]);
  ...
  const handlePlay = () => { if (currentIndex >= logs.length - 1) setCurrentIndex(0); setIsPlaying(true); };
    ...
    <div className="h-full w-full flex flex-col">
      <div className="flex-1 relative">
        <MapContainer center={positions[0]} zoom={14} style={{ height: "100%", width: "100%" }} zoomControl={false}>
          ...
          <Polyline positions={positions} pathOptions={{ color: '#cbd5e1', weight: 2, opacity: 0.6, dashArray: '6 4' }} />
          {trailPositions.length > 1 && <Polyline positions={trailPositions} pathOptions={{ color: '#2570eb', weight: 3.5, opacity: 0.9 }} />}
        ...
        <div className="absolute top-4 left-4 z-[1000]">
          <div className="bg-white/90 backdrop-blur-md border border-border rounded-2xl p-4 min-w-[200px] shadow-lg">
      ...
      <div className="bg-white border-t border-border px-6 py-4">
        <div className="mb-3">
          <input type="range" min={0} max={logs.length - 1} value={currentIndex}
        ...
        <div className="flex items-center justify-between">
          ...
          <button onClick={() => setFollowMarker(f => !f)} className={`px-3 py-2 rounded-xl text-xs font-semibold border transition-all ${followMarker ? 'bg-primary/8 border-primary/15 text-primary' : 'bg-surface-alt border-border text-text-muted'}`}>{followMarker ? '📍 Following' : '🔓 Free'}</button>
